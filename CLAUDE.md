...
### Contexto vivo del repo

Antes de empezar trabajo local en este repo, además de sincronizar
`wms-brain`, leer:

```
C:\Users\yejc2\source\repos\TOMWMS\codex-context-mi3-di-estatus.yml
```

Ese YAML es un contexto incremental para cambios API/MI3 recientes: mecánica
por capas, endpoint `mi3/di-estatus`, reglas de respuesta JSON Forma A,
archivos tocados, validaciones y pendientes. Si durante el trabajo se aprende
una regla útil o cambia el diseño implementado, actualizar ese YAML con una
nota breve para que la siguiente sesión no arranque desde cero.
