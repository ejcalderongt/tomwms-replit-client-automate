...
Public Class frmRegistrosPendPush

    Public ListRegistros As New List(Of clsBeI_nav_transacciones_push)
    ...
    Private Sub SetDatataTable()
        ...
        DT.Columns.Add("IdTransaccionPush", GetType(Integer))
        DT.Columns.Add("No. OrdenCompraEnc", GetType(Integer))
        DT.Columns.Add("No. RecepciónEnc", GetType(Integer))
        DT.Columns.Add("No. RecepciónDet", GetType(Integer))
        ...
        DT.Columns.Add("Tipo Transacción", GetType(String))
        ...
        DT.Columns.Add("Tipo Push", GetType(String))
        DT.Columns.Add("No. Recepción Almacén", GetType(String))
        DT.Columns.Add("Documento Ubicación", GetType(String))
        DT.Columns.Add("Documento Ingreso", GetType(String))
        DT.Columns.Add("Documento Recepción", GetType(String))
    ...
    Private Sub Cargar_Datos_RegistrosInterface()
        ...
        Try
            ...
            ListRegistros = clsLnI_nav_transacciones_push.Get_All_By_Fecha(dtpFechaDel.Value, dtpFechaAl.Value, chkPendientesEnvio.Checked)

            DT.Clear()

            If ListRegistros.Count > 0 Then

                For Each Obj As clsBeI_nav_transacciones_push In ListRegistros
                    ...
                    If Obj.Idpresentacion > 0 Then
                        ...
                        IdxPresentacion = lPresentaciones.FindIndex(Function(x) x.IdPresentacion = Obj.Idpresentacion)
                    ...
                    Dim BeProducto As New clsBeProducto With {.IdProducto = Obj.Idproducto}
                    clsLnProducto.GetSingle(BeProducto)

                    DT.Rows.Add(Obj.IdTransaccionPush, Obj.IdOrdenCompra, Obj.IdRecepcionEnc, Obj.IdRecepcionDet, Obj.No_linea,
                                BeProducto.Codigo, BeProducto.Nombre, BeProducto.UnidadMedida.Nombre, Presentacion.Nombre, Obj.Tipo_transaccion,
                                Obj.Lote, VCantidadUMbas, vCantidadPres, Obj.Peso, Obj.Fecha_vence, Obj.Enviado_A_ERP, Obj.Tipo_push,
                                Obj.No_recepcion_almacen, Obj.Documento_ubicacion, Obj.Documento_ingreso, Obj.Documento_recepcion, Obj.Respuesta_push)
                Next
    ...
    Private Sub Imprimir_Vista()
        ...
        Try
            ...
            Dim leftColumnHead As String = "Usuario: [User Name] - "
    ...
    Private Sub chkPendientesEnvio_CheckedChanged(sender As Object, e As EventArgs) Handles chkPendientesEnvio.CheckedChanged
        Cargar_Datos_RegistrosInterface()
    End Sub

    Private Sub mnuMarcarEnviadoERP_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mnuMarcarEnviadoERP.ItemClick

        Try

            Dim Dr As DataRowView = GridView1.GetFocusedRow
            Dim lSelectionIndex As Integer = GridView1.FocusedRowHandle
            Dim IdTransaccionPush As Integer = IIf(IsDBNull(Dr.Item("IdTransaccionPush")), 0, Dr.Item("IdTransaccionPush"))
            Dim BeINavTransaccionesPush As New clsBeI_nav_transacciones_push

            If Dr Is Nothing Then Return

            Dim vMensaje As String = "¿Marcar registro como enviado a ERP?"

            If XtraMessageBox.Show(vMensaje, Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then

                If Not IdTransaccionPush = 0 Then

                    BeINavTransaccionesPush.IdTransaccionPush = IdTransaccionPush
                    ...
                    XtraMessageBox.Show("Se marcó registro como enviado a ERP",
                                        Text,
                                        MessageBoxButtons.OK,
