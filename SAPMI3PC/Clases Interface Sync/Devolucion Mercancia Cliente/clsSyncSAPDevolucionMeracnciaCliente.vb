...
Public Class clsSyncSAPDevolucionMeracnciaCliente : Inherits clsInterfaceBase
    ...
    Public Function Importar_Devolucion_Cliente_SAP_A_TI_WMS(ByVal lblprg As RichTextBox,
        ...
        Try
            ...
            lNotasCreditoCliente = Get_Sol_Devol_Cli_From_SAP()
            ...
            If clsLnI_nav_ped_compra_det.EliminarTodos(lConnection, lTransaction) _
                ...
                For Each PC In lNotasCreditoCliente
                    ...
                    If clsLnI_nav_ped_compra_enc.Procesar_Pedido_Compra_MI3(PC, BePedidoCompraEnc, vResult) Then
                        Marcar_Sol_Devol_Cli_Sincronizado_SAP(PC.No)
                    End If
    ...
    Public Function Insertar_NC_Cliente_Desde_Tabla_Intermedia_A_Tabla_TOMWMS(ByRef lblprg As RichTextBox,
        ...
        Try
            ...
            clsPublic.Actualizar_Progreso(lblprg, "Consultando Devoluciones de clienteen tabla intermedia ")

            lPedidoCompraEnc = clsLnI_nav_ped_compra_enc.GetAll(CnnInterface, lTransInterface, lblprg, prg)

            clsPublic.Actualizar_Progreso(lblprg, String.Format("Devoluciones de clienteen tabla intermedia: {0}", lPedidoCompraEnc.Count))
...
#End Region
            ...
            clsPublic.Actualizar_Progreso(lblprg, String.Format("Devoluciones de clienteprocesados  correctamente: {0}", VContadorBitacoraTOMWMS))
    ...
    Public Function Get_Sol_Devol_Cli_From_SAP(Optional ByVal AplicarFiltros As Boolean = True) As List(Of clsBeI_nav_ped_compra_enc)
                ...
                vSQLOC = " SELECT T0.DOCENTRY,
                           ...
                           CASE WHEN T0.DOCTYPE = 'I'THEN 'ARTICULO'    
                            ...
                            (SELECT TOP 1 D0.WhsCode 
                                FROM RRR1 D0 INNER JOIN OWHS D1 ON D1.WhsCode = D0.WhsCode WHERE D0.DocEntry = T0.DOCENTRY) AS BODEGA,
                            T0.COMMENTS,
                            T0.NumAtCard
                                FROM ORRR T0  
                ...
                While RsEnc.EoF = False
                    ...
                    query_det = "SELECT 
                                ...
                                FROM 
                                    RRR1 T0
                                INNER JOIN 
                                    OITM T1 ON T1.ItemCode = T0.ItemCode
                                INNER JOIN 
                                    UGP1 U1 ON T1.IUoMEntry = U1.UomEntry
                                INNER JOIN 
                                    OUGP U0 ON U1.UgpEntry = U0.UgpEntry
                                INNER JOIN 
                                    OUOM T2 ON U1.UomEntry = T2.UomEntry 
    ...
    End Function
    Public Function Marcar_Sol_Devol_Cli_Sincronizado_SAP(ByVal pNoDocumento As String) As Boolean

        Marcar_Sol_Devol_Cli_Sincronizado_SAP = False
        ...
        Try
            ...
            Else
                ...
                Dim oNCSBO As Documents = CType(oCompany.GetBusinessObject(BoObjectTypes.oGoodsReturnRequest), Documents)
