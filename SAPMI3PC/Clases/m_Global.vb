...
Module m_Global
    ...
    Public Property gVersionApp As String = "6.8.4"
    Public Property gFechaVersion As Date = New Date(2024, 4, 10)
    ...
    Public Function Desconectar_SAP(ByRef oCompany As Company) As Boolean
        ...
        Dim intentos As Integer = 3  ' Número de intentos de desconexión

        Try

            If Not IsNothing(oCompany) Then

                While oCompany.Connected And intentos > 0
                    oCompany.Disconnect()
                    intentos -= 1
                    Threading.Thread.Sleep(1000) ' Espera un segundo antes de reintentar
                End While

                ' Verifica si la desconexión fue exitosa
                If Not oCompany.Connected Then
                    ' Liberar el objeto COM
                    System.Runtime.InteropServices.Marshal.ReleaseComObject(oCompany)
                    oCompany = Nothing

                    ' Forzar la recolección de basura para asegurar que los objetos COM sean liberados
                    GC.Collect()
                    GC.WaitForPendingFinalizers()

                    resultado = True
                Else
                    ' Si la desconexión no fue exitosa después de los intentos
                    Throw New Exception("No se pudo desconectar de SAP después de los intentos.")
                End If

            End If

        Catch ex As Exception
            ' Incluye más detalles en la excepción lanzada
            Throw New Exception(String.Format("Error al desconectar SAP: Método - {0}, Mensaje - {1}, Pila - {2}", MethodBase.GetCurrentMethod().Name, ex.Message, ex.StackTrace))
        End Try
    ...
    Public Function Conectar_A_SAP(ByRef oCompany As Company,
                                   ...
                                   Optional ForceExecution As Boolean = False) As Boolean
        ...
        Try
            ...
            If (Not oCompany.Connected) OrElse ForceExecution Then
