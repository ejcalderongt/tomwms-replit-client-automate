    ...
    Public Function Get_Existencias_From_SAP_AsDataTable(ByVal IdBodega As Integer, ByVal IdUsuarioAgr As Integer) As DataTable
        ...
        Try
            ...
            If Conectar_A_SAP(oCompany, False, lErrCode, sErrMsg) Then
                ...
                Dim query As String = "SELECT Codigo_Producto Codigo, Total_Almacen Cantidad_Lote, Lote, Fecha_Vence,Codigo_Bodega codigo_area, UmBas
                                       FROM VW_STOCK_POR_LOTE WHERE Codigo_Bodega = '" & Strings.Right("00" & IdBodega, 2) & "'"

                oRecordset.DoQuery(query)

                cTrans.Begin_Transaction()

                Dim BeInvTeoricoERP As New clsBeTrans_inv_teorico_erp
                ...
                While Not oRecordset.EoF
                    ...
                    BeInvTeoricoERP = New clsBeTrans_inv_teorico_erp
                    BeInvTeoricoERP.Idinvteoricoerp = clsLnTrans_inv_teorico_erp.MaxID(cTrans.lConnection, cTrans.lTransaction) + 1
                    BeInvTeoricoERP.Codigo = oRecordset.Fields.Item("Codigo").Value
                    BeInvTeoricoERP.IdProducto = clsLnProducto.Get_IdProducto_By_Codigo(BeInvTeoricoERP.Codigo, cTrans.lConnection, cTrans.lTransaction)
                    BeInvTeoricoERP.IdPresentacion = 0
                    BeInvTeoricoERP.Cant = oRecordset.Fields.Item("Cantidad_Lote").Value
                    BeInvTeoricoERP.Peso = 0
                    BeInvTeoricoERP.IdUnidadMedida = clsLnUnidad_medida.Get_IdUnidadMedida_By_Codigo(oRecordset.Fields.Item("Umbas").Value, cTrans.lConnection, cTrans.lTransaction)
                    BeInvTeoricoERP.Lote = oRecordset.Fields.Item("Lote").Value
                    BeInvTeoricoERP.Fecha_vence = IIf(oRecordset.Fields.Item("Fecha_Vence").Value = "19000101", New Date(1900, 1, 1), oRecordset.Fields.Item("Fecha_Vence").Value)
                    BeInvTeoricoERP.Codigo = oRecordset.Fields.Item("Codigo").Value
                    BeInvTeoricoERP.Idbodega = IdBodega
                    BeInvTeoricoERP.Idubicacion = 0
                    BeInvTeoricoERP.Lic_plate = ""
                    BeInvTeoricoERP.Codigo_area = oRecordset.Fields.Item("codigo_area").Value
                    BeInvTeoricoERP.Fecha_agr = Now
                    BeInvTeoricoERP.Usuario_agr = IdUsuarioAgr
