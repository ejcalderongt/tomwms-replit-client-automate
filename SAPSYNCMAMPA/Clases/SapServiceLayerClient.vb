...
Public Class SapServiceLayerClient
    ...
    End Function
    'Public Async Function Procesar_Detalle_Ingreso_HANA(oOrderPurchase As FacturaReservaDto,
    ...
    '            Dim vProductoTc = docLine.ItemCode & docLine.U_Talla & docLine.U_Color
    ...
    '            If vGeneroDocumentoFaltante Then
    ...
    Public Async Function Procesar_Detalle_Ingreso_HANA_Entrega_Consolidada(oOrderPurchase As FacturaReservaDto,
                                                                            BeINavConfigEnc As clsBeI_nav_config_enc,
                                                                            BeTransOCEnc As clsBeTrans_oc_enc,
                                                                            IdRecepcionEnc As Integer,
                                                                            lINavTransaccionesOut As List(Of clsBeI_nav_transacciones_out),
        ...
        Try
            Dim vIdRecepcionEnc As Integer = lINavTransaccionesOut.FirstOrDefault().Idrecepcionenc
            BeTransReOc = clsLnTrans_re_oc.Get_Single_By_IdOrdenCompraEnc_And_IdRecepcionEnc(BeTransOCEnc.IdOrdenCompraEnc,
                                                                                             vIdRecepcionEnc,
                                                                                             lConnection,
                                                                                             lTransaction)
            ...
            '--- Documento ÚNICO de entrega (GRPO/OPDN) ---
            Dim comentarioBase As String =
            $"{oOrderPurchase.Comments} - Entrega generada desde WMS IdRecepcion: {IdRecepcionEnc} IdOcEnc: {BeTransOCEnc.IdOrdenCompraEnc}"
            If comentarioBase Is Nothing Then comentarioBase = ""
            If comentarioBase.Length > 250 Then comentarioBase = comentarioBase.Substring(0, 250)

            Dim entrega As New FacturaReservaEntregaDto With {
            .CardCode = oOrderPurchase.CardCode,
            .U_LOGISTIKA_ID = vIdRecepcionEnc,         ' <- Asegúrate que exista en OPDN
            .DocDate = Date.Today,                     ' Json.NET serializa ISO (yyyy-MM-dd)
            .DocDueDate = Date.Today,
            .Comments = comentarioBase,
            .DocumentLines = New List(Of FacturaReservaEntregaLineDto)()
        }

            ' Helper: agrega o fusiona línea por clave (incluye base del PEDIDO)
            Dim AddOrMergeLine As Action(Of FacturaReservaEntregaLineDto) =
        Sub(newLine As FacturaReservaEntregaLineDto)
            Dim existing = entrega.DocumentLines.FirstOrDefault(Function(x) _
                x.BaseType = newLine.BaseType AndAlso
                x.BaseEntry = newLine.BaseEntry AndAlso
                x.BaseLine = newLine.BaseLine AndAlso
                String.Equals(x.ItemCode, newLine.ItemCode, StringComparison.OrdinalIgnoreCase) AndAlso
                String.Equals(x.WarehouseCode, newLine.WarehouseCode, StringComparison.OrdinalIgnoreCase) AndAlso
            ...
            Else
                existing.Quantity += newLine.Quantity
                If newLine.BatchNumbers IsNot Nothing AndAlso newLine.BatchNumbers.Count > 0 Then
                    If existing.BatchNumbers Is Nothing Then existing.BatchNumbers = New List(Of BatchNumberDto)()
                    For Each bn In newLine.BatchNumbers
                        Dim exBn = existing.BatchNumbers.FirstOrDefault(
                            Function(b) String.Equals(b.BatchNumber, bn.BatchNumber, StringComparison.OrdinalIgnoreCase))
                        If exBn Is Nothing Then
                            existing.BatchNumbers.Add(New BatchNumberDto With {.BatchNumber = bn.BatchNumber, .Quantity = bn.Quantity})
                        Else
                            exBn.Quantity += bn.Quantity
        ...
        End Sub
            ...
            '--- Recorre líneas de la FACTURA y usa SIEMPRE el PEDIDO como base ---
            For Each docLine In oOrderPurchase.DocumentLines

                ' Validación estricta de base
                If docLine.BaseType <> 22 OrElse docLine.BaseEntry <= 0 Then
                    Throw New Exception($"❌ La línea {docLine.LineNum} no está basada en Pedido: BaseType={docLine.BaseType}, BaseEntry={docLine.BaseEntry}")
                End If

                Dim vCodigoProductoSAP As String = docLine.ItemCode
                Dim vNoLineaPedidoSAP As Integer = docLine.LineNum             ' <- línea del PEDIDO (no la de la factura)
                Dim vNoLineaBaseOC As Integer = docLine.BaseLine

                ' Ids auxiliares (tu lógica actual)
                Dim vIdProducto As Integer = clsLnProducto.Get_IdProductoBodega_By_Codigo(vCodigoProductoSAP, lConnection, lTransaction)
                Dim vIdProductoBodega As Integer = clsLnProducto_bodega.Get_IdProductoBodega_By_IdProducto_And_IdBodega(vIdProducto, BeTransOCEnc.IdBodega)

                ' Composición talla/color para búsquedas internas
                Dim vProductoTc As String = vCodigoProductoSAP & docLine.U_Talla & docLine.U_Color

                ' <- Ajustado a BaseLine del pedido
                Dim BeTransOcDet = clsLnTrans_oc_det.Get_Single_By_Recepcion_Det_For_Inav(BeTransOCEnc.IdOrdenCompraEnc,
                                                                                          vIdProductoBodega,
                                                                                          vNoLineaPedidoSAP,
                                                                                          vProductoTc,
                                                                                          lConnection,
                ...
                ' Agrupa recibido en WMS por línea de PEDIDO
                Dim DistinctProductosLineas = lINavTransaccionesOut.
                Where(Function(x) x.Codigo_producto = vCodigoProductoSAP AndAlso x.No_linea = vNoLineaPedidoSAP).
                GroupBy(Function(x) New With {
                    ...
                    Key .Idrecepcionenc = x.Idrecepcionenc,
                    Key .Idproductobodega = x.Idproductobodega
                }).
                Select(Function(g) New With {
                    g.Key.Codigo_producto,
                    g.Key.No_linea,
                ...
                If DistinctProductosLineas.Any() Then
                    For Each ProductoIngreso In DistinctProductosLineas
                        ...
                        Dim vColor As String = ""
                        Dim vTalla As String = ""
                        Dim DT As DataTable = clsLnProducto_talla_color.Get_All_Dt_By_IdProductoTallaColor(
                        ProductoIngreso.IdProductoTallaColor, lConnection, lTransaction)
                        If DT IsNot Nothing AndAlso DT.Rows.Count > 0 Then
                            vColor = DT.Rows(0)("Color").ToString()
                            vTalla = DT.Rows(0)("Talla").ToString()
                        End If
                        ...
                        ' Línea a bodega de recibido / importación (BASE: PEDIDO)
                        If qtyRecibida > 0 Then
                            AddOrMergeLine(New FacturaReservaEntregaLineDto With {
                            .BaseType = 22,
                            .BaseEntry = docLine.BaseEntry,
                            .BaseLine = vNoLineaPedidoSAP,
                            .ItemCode = vCodigoProductoSAP,
                            .Quantity = qtyRecibida,
                            .WarehouseCode = If(vEsImportacion, vCodigoBodegaImportacion, docLine.WarehouseCode),
                            .U_Color = vColor,
                        ...
                        ' Línea a bodega de faltantes (BASE: PEDIDO)
                        If qtyFaltante > 0 Then
                            If String.IsNullOrWhiteSpace(BeINavConfigEnc.Bodega_Faltante) Then
                                Throw New Exception("❌ ERROR: No está configurada la bodega de faltantes en la configuración de integración.")
                            End If
                    ...
                    Next
                Else
                    ' No hubo recepción: todo a bodega de faltantes (BASE: PEDIDO)
                    If String.IsNullOrWhiteSpace(BeINavConfigEnc.Bodega_Faltante) Then
                        Throw New Exception("❌ ERROR: No está configurada la bodega de faltantes en la configuración de integración.")
                    End If
            ...
            Using httpClient As New HttpClient(handler) With {.BaseAddress = New Uri(baseUrl)}
                Try
                    ' Forzar fechas ISO (yyyy-MM-dd) por si tu Json.NET no está globalizado así
                    Dim jsonSettings As New JsonSerializerSettings With {
                    .DateFormatString = "yyyy-MM-dd"
                }
                    Dim entregaJson As String = JsonConvert.SerializeObject(entrega, jsonSettings)
                    ...
                    postRequest.Headers.Add("Cookie", $"B1SESSION={SessionId}; ROUTEID={RouteId}")
                    postRequest.Headers.ConnectionClose = True
                    postRequest.Headers.Accept.Add(New Headers.MediaTypeWithQualityHeaderValue("application/json"))
                    ' NO forzar Host: puede causar 400 en algunos entornos
                    ' postRequest.Headers.Host = "hanab1"
                    ...
                    $"Se envió la entrega a SAP para IdOrdenCompraEnc: {BeTransOCEnc.IdOrdenCompraEnc} NoDocumento: {docNum} DocEntry: {docEntry}")
    ...
    Public Async Function Procesar_Detalle_Ingreso_HANA_Entregas_Separads(oOrderPurchase As FacturaReservaDto,
                                                                          BeINavConfigEnc As clsBeI_nav_config_enc,
                                                                          BeTransOCEnc As clsBeTrans_oc_enc,
                                                                          IdRecepcionEnc As Integer,
                                                                          lINavTransaccionesOut As List(Of clsBeI_nav_transacciones_out),
