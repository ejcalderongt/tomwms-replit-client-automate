...
Partial Public Class clsLnTrans_ajuste_enc
    ...
    Public Shared Sub Guardar_Borrador_Ajuste(ByVal BeAjusteEnc As clsBeTrans_ajuste_enc,
        ...
        Try
            ...
            '#FIX_v9_BORRADOR_RESERVA_2026-04-25 (C1):
            ...
            'inválido al reanudar. La convención del sistema es: stock_res se
            'mantiene durante edición/borrador y se libera en Aplicar_Ajuste vía
            'Eliminar_All_Stock_Ajuste (línea 91 de este archivo).
    ...
    '#FIX_v9_BORRADOR_RESERVA_2026-04-25 (C1 helper):
    'Reserva un stock asociado a un ajuste en estado borrador. Replica la lógica
    'de frmAjusteStock.Reservar_Stock (línea 1041) pero a nivel DAL puro y con
    'Indicador = "ajuste_stock_borrador" para distinguir trazabilidad de las
    'reservas creadas por aplicación normal ("ajuste_stock").
    '
    'Convivencia con flujo manual: las reservas creadas por frmStockList con
    'Indicador "ajuste_stock" se borran junto con las de "ajuste_stock_borrador"
    'porque Eliminar_All_Stock_Ajuste filtra solo por IdTransaccion. Esto es
    'correcto: al re-guardar el borrador queremos el snapshot atómico actual.
    ...
    '#FIX_v9_BORRADOR_RESERVA_2026-04-25 (C3 helper para reanudación):
    'Versión standalone (sin BeAjusteEnc completo) para recrear una reserva
    'individual al cargar un borrador donde alguna reserva fue eliminada.
    'Se invoca desde frmAjusteStock.Reservar_Stock_Borrador_Standalone.
    'Devuelve True si la reserva pudo crearse, False si el stock ya no existe
    'o no tiene cantidad disponible.
