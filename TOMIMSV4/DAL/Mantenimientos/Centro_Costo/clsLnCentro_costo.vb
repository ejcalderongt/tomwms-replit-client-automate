...
Public Class clsLnCentro_costo
    ...
    Public Shared Function Get_CentroCosto_By_IdBodega(ByVal IdBodega As Integer) As String

        Get_CentroCosto_By_IdBodega = ""

        Try

            Const sp As String = "SELECT cc.Codigo 
                                  FROM bodega b inner join Centro_costo cc ON 
                                            cc.IdCentroCosto = b.centro_costo_erp
                                  WHERE(b.IdBodega = @IdBodega) and cc.Referencia =1 "
            ...
            Using lConnection As New SqlConnection(connectionString:=Configuration.ConfigurationManager.AppSettings("CST"))
                ...
                Using lTransaction As SqlTransaction = lConnection.BeginTransaction(IsolationLevel.ReadUncommitted)
                    ...
                    Using lDTA As New SqlDataAdapter(sp, lConnection)
                        ...
                        If lDataTable IsNot Nothing AndAlso lDataTable.Rows.Count > 0 Then
                            Dim dr As DataRow = lDataTable.Rows(0)
                            Get_CentroCosto_By_IdBodega = IIf(IsDBNull(dr.Item("codigo")), "", dr.Item("codigo"))
                        End If
