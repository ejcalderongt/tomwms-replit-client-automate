...
Partial Public Class clsLnRegla_ubic_det_tr
    Implements IDisposable

    Public Shared Function GetAllByIdReglaUbicacionEnc(ByVal IdReglaUbicacionEnc As Integer) As List(Of clsBeRegla_ubic_det_tr)
    ...
    Public Shared Function Aplicar_Regla_Por_TipoRotacion(ByVal IdBodega As Integer,
                                                          ByVal IdTipoRotacion As Integer,
                                                          ByRef lConnection As SqlConnection,
                                                          ByRef lTransaction As SqlTransaction) As Boolean

        Aplicar_Regla_Por_TipoRotacion = False

        Try

            Dim vSQL As String = "SELECT regla_ubic_enc.IdBodega, regla_ubic_det_tr.IdTipoRotacion 
                                   FROM regla_ubic_det_tr INNER JOIN 
                                   regla_ubic_enc ON regla_ubic_det_tr.IdReglaUbicacionEnc = regla_ubic_enc.IdReglaUbicacionEnc 
                                   Where(regla_ubic_enc.IdBodega = @IdBodega AND regla_ubic_det_tr.IdTipoRotacion = @IdTipoRotacion AND regla_ubic_det_tr.Activo = 1) "
            ...
            dad.SelectCommand.Parameters.Add(New SqlParameter("@IdTipoRotacion", IdTipoRotacion))
            Dim dt As New DataTable

            dad.Fill(dt)

            Aplicar_Regla_Por_TipoRotacion = (dt.Rows.Count > 0)
