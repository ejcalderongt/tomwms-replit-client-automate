...
"
    ...
    Private Sub Refrescar_Charts()
        ...
        If DT_TipoProducto IsNot Nothing AndAlso DT_TipoProducto.Rows.Count > 0 Then
            Dim sPos As New DevExpress.XtraCharts.Series("Positivos", ViewType.Bar)
            Dim sNeg As New DevExpress.XtraCharts.Series("Negativos", ViewType.Bar)
            ...
            chartTipoProducto.Series.AddRange(New DevExpress.XtraCharts.Series() {sPos, sNeg})
        End If

        ' Tendencia: línea cantidad neta
        chartTendencia.Series.Clear()
        If DT_Tendencia IsNot Nothing AndAlso DT_Tendencia.Rows.Count > 0 Then
            Dim sNeta As New DevExpress.XtraCharts.Series("Cant. Neta", ViewType.Line)
            Dim sPos As New DevExpress.XtraCharts.Series("Positivos", ViewType.Line)
            Dim sNeg As New DevExpress.XtraCharts.Series("Negativos", ViewType.Line)
            ...
            chartTendencia.Series.AddRange(New DevExpress.XtraCharts.Series() {sNeta, sPos, sNeg})

            ' Configurar eje X como DateTime
            Dim diagram = TryCast(chartTendencia.Diagram, XYDiagram)
