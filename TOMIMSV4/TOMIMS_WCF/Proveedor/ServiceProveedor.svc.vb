...
Public Class ServiceProveedor
    ...
    ''' <exception cref="FaultException"></exception>
    ''' <exception cref="System.Exception"></exception>
    Public Function Get_All_By_IdBodega(ByVal pActivo As Boolean, ByVal pIdBodega As Integer) As List(Of clsBeProveedor) Implements IServiceProveedor.Get_All_By_IdBodega

        Try
            Return clsLnProveedor.GetAllFiltro(pActivo, pIdBodega).ToList
        Catch ex As FaultException
            Throw New FaultException(String.Format("{0} {1}", MethodBase.GetCurrentMethod().Name, ex.Message))
        Catch ex1 As Exception
            Throw New Exception(String.Format("{0} {1}", MethodBase.GetCurrentMethod().Name, ex1.Message))
    ...
    Public Function Get_All_Filtro(ByVal pActivo As Boolean, ByVal pCodBodega As String) As List(Of clsBeProveedor) Implements IServiceProveedor.Get_All_By_CodBodega

        Try

            Dim IdBodega As Integer = clsLnBodega.Get_IdBodega_By_CodigoBodega(pCodBodega)

            If IdBodega <> 0 Then
                Return clsLnProveedor.GetAllFiltro(pActivo, IdBodega).ToList
            Else
                Throw New Exception("El código de bodega: " & pCodBodega & " no existe en el maestro de bodegas")
            End If
