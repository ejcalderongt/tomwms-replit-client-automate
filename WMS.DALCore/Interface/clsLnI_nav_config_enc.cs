...
public static void Cargar(ref clsBeI_nav_config_enc oBeI_nav_config_enc, DataRow dr)
{
    ...
    public static clsBeI_nav_config_enc? GetSingle(IConfiguration config,clsBeI_nav_config_enc pBeI_nav_config_enc,SqlConnection? pConection = null,SqlTransaction? pTransaction = null)
    {
        SqlConnection lConnection = new SqlConnection(config.GetConnectionString("CST"));
        SqlTransaction? lTransaction = null;
        ...
        try
        {
            ...
            var cmd = new SqlCommand(sp, lConnection) { CommandType = CommandType.Text };
            ...
            {
                DataRow r = dt.Rows[0];
                Cargar(ref pBeI_nav_config_enc, r); // carga los datos en el objeto
                return pBeI_nav_config_enc;         // retorna el objeto cargado
            }
            else
            {
