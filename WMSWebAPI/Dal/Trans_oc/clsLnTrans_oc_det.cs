...
public class clsLnTrans_oc_det
{
    ...
    public static void InsertarOActualizar(IConfiguration config, List<clsBeTrans_oc_det> entities, SqlConnection? conn = null, SqlTransaction? tx = null)
    {
        ...
        try
        {
            ...
            {
                bool existe = Existe(entity.IdOrdenCompraDet, connection, isExternalTx ? tx! : localTx!);
    ...
    public static bool Existe(int IdOrdenCompraDet, SqlConnection pConnection, SqlTransaction pTransaction)
    {
        try
        {
            const string query = "SELECT COUNT(1) FROM Trans_oc_det WHERE IdOrdenCompraDet = @IdOrdenCompraDet";
