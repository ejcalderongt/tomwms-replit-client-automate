...
Imports TOMWMS               ' clsBeBodega, clsLnBodega (DAL.dll + Entity.dll del WMS)
Imports WmsBodegaApi.WebApi.Dtos

Namespace Controllers

    ''' <summary>
    ''' Wrapper HTTP/JSON sobre TOMWMS.clsLnBodega (clase Shared/static de la DAL).
    ''' Cubre los 8 métodos que frmBodega.vb invoca:
    '''
    '''   1) MaxID()                           — frm:290, 1383
    '''   2) Obtener(ByRef pBe)                — frm:942
    '''   3) Exists_By_IdEmpresa(idEmp)        — frm:1469
    '''   4) Insertar(ByRef pBe)               — frm:1568
    '''   5) Actualizar(ByRef pBe)             — frm:1815, 2176, 2257
    ...
    Public Class BodegaController
        ...
        Public Function UnificarBodegas(<FromBody> req As UnificarBodegasRequest) As ApiResponse(Of Boolean)
            ...
            Dim ok As Boolean = clsLnBodega.Unificar_Bodegas(req.IdBodegaDestino, req.IdBodegaOrigen)
            Return ApiResponse(Of Boolean).Ok(ok)
        End Function

        ' ----------------------------------------------------------------- helpers
        Private Sub VerifyApiKey()
            Dim expected As String = Configuration.ConfigurationManager.AppSettings("ApiKey")
