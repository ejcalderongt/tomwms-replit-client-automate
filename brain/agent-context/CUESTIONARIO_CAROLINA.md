...
## Bloque 14 — NUEVO (post Wave 10, 2026-04-30)

Estas 14 preguntas surgieron de las **respuestas de Carolina a los Bloques 1 y 2** del cuestionario (11 Q-* RESUELTAS — ver sección "Q-* RESUELTAS WAVE 10" más abajo). Cada respuesta cerró una pregunta y abrió 1-2 nuevas más afiladas.

### Clientes nuevos descubiertos (Carolina mencionó al pasar)

77. **Q-CLARISPHARMA-FINGERPRINT** (alta) — Carolina mencionó CLARISPHARMA en el orden de migración 2028 (junto con Becofarma). ¿Qué es Clarispharma? ¿Sector farmacia (mismo que Becofarma)? ¿Familia / mismo dueño que Becofarma o competidoras del mismo sector? ¿En qué BD vive? ¿Qué ERP usa? ¿Está en EC2 productivo o en otro server?

78. **Q-MERCOSAL-FINGERPRINT** (alta) — Carolina mencionó que ya pasó BD MERCOSAL a Erik junto con MERCOPAN. Por nomenclatura `Mer-co-sal`, hipótesis: **El Salvador**, tercera filial del holding IDEALSA (Honduras + Panamá + El Salvador). ¿Confirmás país? ¿Confirmás holding? ¿Schema espejo de MERHONSA/MERCOPAN (~315 tablas comunes)? ¿Productos perecederos también (aceite + detergentes)?

### CEALSA y migración Odoo

79. **Q-CEALSA-ARITEC-ERP-NOMBRE** (media) — Carolina dijo: "CEALSA utilizaba un ERP que no recuerdo su nombre, pero lo desarrolló ARITEC y en este momento tienen pensado migrarse para Odoo". ¿Cuál es el nombre exacto del ERP de ARITEC? ¿Hay alguna documentación del producto?

80. **Q-CEALSA-MIGRACION-ODOO-TIMELINE** (media) — ¿Cuándo arranca CEALSA con Odoo? ¿Qué impacto tiene en `CEALSAMI3/`? Opciones: (a) reescribir CEALSA encima de MI3 SOAP genérico (alinear con IDEALSA/INELAC), (b) usar conector estándar Odoo↔WMS (REST/XMLRPC), (c) mantener CEALSAMI3 actualizando endpoints para Odoo. ¿Hay decisión preliminar?

### Holdings y migración

81. **Q-DEV2025-COMMIT-DRIFT** (baja, técnica) — Carolina confirmó que `dev_2025` fue rama intermedia para talla×color MAMPA, mergeada a `dev_2028_merge` y ahora histórica. PERO Wave 6.2 mostró que el HEAD visible de `dev_2025` apunta al MISMO commit que `dev_2023_estable` (`1f5cc2c4`). Si `dev_2025` tuvo trabajo de talla×color propio, ¿por qué su HEAD no refleja ese trabajo? Hipótesis: el trabajo se mergeó (squash) a `dev_2028_merge` y `dev_2025` quedó congelado en su commit base. ¿Confirmás?

82. **Q-INELAC-EN-HOLDING-IDEALSA** (media) — Carolina confirmó que MI3 lo usan IDEALSA + INELAC + MERCOPAN. ¿INELAC es del holding IDEALSA (junto con MERHONSA, MERCOPAN, probable MERCOSAL) o es cliente independiente que solo casualmente usa MI3? Importa para entender si la familia centroamericana tiene 3, 4 o más filiales y si INELAC comparte master data con las otras.

83. **Q-MIGRATION-SCRIPTS-VERSIONADO** (alta — bus factor) — Carolina dijo: "Si tengo un script de migración de base de datos, lo tengo en mi compu y es el que utilizo para actualizar las BD, generalmente busco en SQL Server la última vista actualizada y por eso me guío". Esto es deuda técnica + riesgo de bus factor: si Carolina no está disponible, el equipo no puede migrar BDs. ¿Se puede versionar este script en un repo (ej: nuevo repo `tomwms-db-migrations` o carpeta `TOMWMS_DBA/migrations/2023-to-2028/`)? ¿Quién lo mantendría además de Carolina?

### Features 2028 sin documentar

84. **Q-RFID-INVENTARIO-2028** (alta) — Carolina mencionó "Incorporación del inventario con RFID" como uno de los 4 breaking changes 2028. ¿Cómo funciona el inventario con RFID? ¿Qué hardware (lectores fijos, móviles, antenas)? ¿Qué clientes lo van a usar? ¿Reemplaza el escaneo de barras o coexiste? ¿Cambia el flujo de recepción / picking / inventario cíclico?

85. **Q-WEBAPI-INTERFACE-MIGRACION-2028** (alta) — Carolina mencionó "Migración de la Interface para un Web API" como otro breaking change 2028. Alcance: ¿qué interfaces específicas se migran (SAP, NAV, MI3, todas)? ¿Reemplaza WSHHRN gradualmente o solo agrega? ¿Coexisten ambas durante la migración o switchover total? ¿WMSWebAPI 2028 es el target?

### Bug implosión Cumbre

86. **Q-IMPLOSION-CUMBRE-FIX-PROPAGAR** (alta — riesgo MAMPA prod) — Carolina confirmó que `dev_2028_Cumbre` está fixeando un bug histórico de implosión: no validaba estado/ubicación, permitía mismo LP en distintas ubicaciones/estados. **Riesgo crítico**: MAMPA YA está en producción con `dev_2028_merge` SIN el fix. Cuando Cumbre se mergee a `dev_2028_merge` y se despliegue update a MAMPA, ¿se hace auditoría retroactiva de LPs corruptos históricos? ¿O se asume "de aquí en adelante limpio"? ¿Hay query SQL que detecte LPs duplicados por ubicación/estado en MAMPA hoy?

### Cleanup repos

87. **Q-RAMAS-HH-CLEANUP** (media) — Carolina dijo: "esas ramas [`byb`, `cealsa`, `mercopan`, `mercosal`, `240byb`, `240Cealsa` en TOMHH2025] se crearon en algún momento para que fueran las ramas de cada cliente, pero esto no fue sostenible y se quedaron como históricas, pero en realidad deberíamos eliminarlas". ¿Se planea sesión de cleanup? Sugerencia: antes de borrar, correr `scan-comments-tree-map` con threshold bajo para asegurarse de no perder comentarios firmados únicos. ¿Se prefiere `git tag archive/<rama>` antes de borrar para conservar historial?

### Huecos del orden de migración 2028

88. **Q-KILLIOS-MIGRACION-PLAN** (alta — hueco) — Carolina enumeró el orden BYB+IDEALSA+INELAC paralelo → Cumbre → BECO+CLARISPHARMA → MHS. **K7 (KILLIOS) NO aparece**. ¿K7 sigue en `dev_2023_estable` indefinidamente? ¿Va a entrar antes que MHS o se mergea con BECOFARMA por similitud SAP B1 (ambos usan SAPBOSync)?

89. **Q-CEALSA-MIGRACION-2028** (media — depende ARITEC→Odoo) — CEALSA tampoco aparece en el orden. Hipótesis: la migración 2028 del WMS se posterga hasta que el lado del ERP estabilice (ARITEC → Odoo). ¿Es así? ¿Hay timing tentativo para CEALSA en 2028 (ej: Q3 2026, 2027)?

90. **Q-IDEALSA-FAMILIA-MIGRACION** (media) — IDEALSA está en el batch de QAS junto con BYB e INELAC. ¿MERCOPAN, MERHONSA y MERCOSAL (las filiales) migran al mismo tiempo que IDEALSA o tienen su propia ventana? ¿Se las trata como un solo "evento de migración holding"?

---

## Resumen actualizado Wave 10

- Total preguntas: **90** (76 antes + 14 nuevas Wave 10)
- 14 bloques temáticos
- Q-* alta prioridad bloqueantes: ~32 (de 90)
- Q-* críticas: 1 (Q15 ChatGPT-Service / OpenAI key leak) + 1 emergente (Q83 bus factor migration scripts)
- **Q-* RESUELTAS WAVE 10** (con respuestas Carolina Bloques 1+2): **11**

---

## Q-* RESUELTAS WAVE 10 — Tabla de cierre

### Bloque 1 (8 RESUELTAS)
- ✅ **Q-MIGRACION-2023-A-2028 (parte 1)** → Orden: BYB+IDEALSA+INELAC paralelo (QAS) → Cumbre → BECO+CLARISPHARMA → MHS. Sin cronograma. **Cita Carolina**: "Estamos llevando en paralelo la migración de B&B, IDEALSA e INELAC… Luego seguimos con La Cumbre. Luego con Becofarma y Clarispharma". Después de MAMPA: MHS al final. → ver `learnings/L-027-proc-orden-migracion-2028.md`
- ✅ **Q-MIGRACION-2023-A-2028 (parte 2)** → Carolina tiene script de migración en su PC, NO está versionado. Riesgo bus factor → Q83 nueva.
- ✅ **Q-MIGRACION-2023-A-2028 (parte 3)** → 4 breaking changes 2028: (1) IDENTITY autoincremental (root cause #1, errores concurrencia recepciones/reservas), (2) talla×color MAMPA, (3) migración Interface→WebAPI, (4) inventario RFID. → ver `learnings/L-026-arch-identities-2028-concurrencia.md`
- ✅ **Q-DEV2025-PROPOSITO** → Rama intermedia para talla×color MAMPA, mergeada a `dev_2028_merge`, ahora histórica. **Cita Carolina**: "Si es correcto, fue una rama Intermedia entre 2023 y 2028, la 2025 fue donde se comenzaron a trabajar los cambios para MAMPA de talla color". → ver `learnings/L-030-arch-dev2025-intermedia-talla-color.md`
- ✅ **Q-MASTER-PROPOSITO** → Release oficial congelada. NO se trabaja ahí. **Cita Carolina**: "Si es una versión congelada, no tiene las actualizaciones de las ramas". → ver `learnings/L-031-arch-master-congelada.md`
- ✅ **Q-HH-RAMAS-25** → Ramas por cliente HH son históricas, modelo no fue sostenible. Recomendación cleanup. **Cita Carolina**: "esto no fue sostenible y se quedaron como históricas, pero en realidad deberíamos eliminarlas". → ver `learnings/L-032-proc-ramas-cliente-hh-obsoletas.md`
- ✅ **Q-MERCOPAN-MERCOSAL** → MERCOPAN ya mapeada en Wave 7. **MERCOSAL es nuevo cliente descubierto** (BD recién pasada a Erik por Carolina), probable El Salvador, probable holding IDEALSA. → ver `learnings/L-034-client-mercosal-existe.md` + Q78 nueva.
- ✅ **Q-CUMBRE-RAMA-DEDICADA** → Rama dedicada para fix de **bug histórico de implosión**: no validaba estado/ubicación. **Cita Carolina**: "en el proceso de implosión nos pasa que no se validaba el estado del producto, ni su ubicación y aún así permitía implosionar, dejando la misma licencia en diferentes ubicaciones y con diferentes estados". → ver `learnings/L-029-fix-implosion-cumbre-validacion.md` + riesgo MAMPA prod (Q86 nueva).

### Bloque 2 (3 RESUELTAS)
- ✅ ⭐ **Q-NAV-PREFIJO-CEALSA** → **EL HALLAZGO MÁS IMPORTANTE DE WAVE 10**: el prefijo `Nav` en código del WMS **NO es Microsoft Dynamics NAV**. Es sinónimo histórico de "ERP / Interface" reusado desde el primer cliente (BYB, sí era NAV literal). Cualquier ERP usa la misma infraestructura `Nav*`. **Cita Carolina**: "ya no lo identificamos con NAV sino que para nosotros es sinónimo de ERP o de Interface". → ver `learnings/L-025-meta-nav-no-es-dynamics.md` + entrada `naked-erik-anatomy/002-2026-04-30-nav-ya-no-es-nav.md`. Reescribe interpretación de TODO código con prefijo Nav.
- ✅ **Q-CEALSA-CEALSASYNC-ERP** → ERP de CEALSA es custom desarrollado por **ARITEC** (consultora externa). En proceso de migración a **Odoo**. Nombre exacto del ERP no recuerda Carolina → Q79 nueva. → ver `learnings/L-028-integ-mi3-consumers-holding.md`
- ✅ **Q-MI3 / Q-MI3-QUE-ES / Q-INTERFACE-MI3** → MI3 = "Módulo de Integración con Terceros", proyecto WCF/SOAP en `TOMWMS_BOF/MI3/`. **Consumers actuales: IDEALSA + INELAC + MERCOPAN**. CEALSA tiene `CEALSAMI3/` standalone propio (diferente). → ver `learnings/L-028-integ-mi3-consumers-holding.md`

---

## Q-* RESUELTAS WAVE 11 — Tabla de cierre (Carolina, bloques 2-cierre, 3, 4, 5, 6, 7, 8, 9)

> Carolina envió segunda tanda de respuestas (continuación Wave 10).
> Cubrió cierre del Bloque 2 (Q12-Q15), parcialmente Bloque 3 (delegado Erik/Efren),
> completo Bloque 4 (stock_jornada), Bloque 5 (DALCore + generador, parcial),
> Bloque 6 (multi-tenancy), Bloque 7 (capabilities + licencias),
> Bloque 8 (web BOF — no sabe), Bloque 9 (bonus).
>
> **Hallazgo crítico Wave 11**: ⭐ **L-035 — NO se codifica por cliente, todo se parametriza**
> (Bodega, Producto, i_nav_config_enc principalmente). Junto con L-025 (Wave 10, NAV no es NAV),
> forma el par de **convenciones arquitectónicas fundamentales** del WMS.

### Bloque 2 cierre (3 RESUELTAS, 1 delegada)
- ✅ **Q-MHS-COMO-CLIENTE** → MHS GoLive **20-ago-2026**, primer WMSWebAPI, scope cerrado: 9 maestros (Productos, Familias, Clasificacion, Tipos, Marcas, Presentaciones, Códigos de barra, Clientes, Proveedores) + 4 transacciones (Ingresos producción, Traslados bodegas, Pedidos clientes, Ajustes inventario). **Erik depurando bug de traslados** (riesgo de slip). → ver `learnings/L-038-client-mhs-golive-scope.md`
- ✅ **Q-WSHHRN-AS-PROXY-BYB** → Hipótesis **revisada**: WSHHRN NO es proxy NAV, es solo bus interno BOF↔HH. **La HH llama DIRECTO a NAV WS por tipo de documento** (OP, Transferencias, Devoluciones), una llamada por licencia + cierre al finalizar. → ver `learnings/L-039-arch-hh-llama-directo-nav-ws.md`
- ✅ **Q-BYB-NO-DISPONIBLE-NAV-BD** → BD NAV **inaccesible** para WMS. Todo via NAV WS (catálogos, transacciones, picking). Sin replica, sin linked server. → ver `learnings/L-040-arch-bd-nav-inaccesible-solo-ws.md`
- 🔄 **Q-CHATGPT-SERVICE** → Lo creó Efren (GT). Funciona pero **no está activo en TOMWMS**. **Delegada Efren** para detalle de uso real. **Cita Carolina**: "Las respuestas de este proyecto considero que los puede responder Efren".

### Bloque 3 — Portal CEALSA / DMS (0 RESUELTAS, 9 delegadas Erik/Efren)
**Cita Carolina**: "Esto lo pueden responder Erik o Efren". Q16-Q24 quedan abiertas para Wave 12.

### Bloque 4 — Stock_jornada (4 RESUELTAS, completo)
- ✅ **Q-STOCK-JORNADA-CONSUMER** → CEALSA: SAT, Aseguradora, módulo facturación, portal CEALSA, **reporte "Stock en línea"** ("super importante" según Carolina). → ver `learnings/L-037-client-stock-jornada-cealsa-consumers.md`
- ✅ **Q-STOCK-JORNADA-MAMPA** → Bandera **`Empresa.generar_stock_jornada`** (true/false) controla. NO es propiedad regulatoria del cliente. MAMPA tiene `true`, BECO/K7/BYB tienen `false`. **Cita Carolina**: "El stock jornada se habilita con la bandera que está en Empresa". → ver `learnings/L-036-arch-stock-jornada-trigger.md`
- ✅ **Q-STOCK-JORNADA-PROCESO** → Disparado al **primer login del día** al WMS. Si no se cerró, se programa para **`Bodega.horario_ejecucion_historico`**. **No hay scheduler externo, el operador es el cron**. → ver `learnings/L-036-arch-stock-jornada-trigger.md`
- ✅ **Q-STOCK-JORNADA-DESFASE** → Proceso interno automático del TOMWMS (componente concreto pendiente Q nueva). → ver `learnings/L-036` (parcial)

### Bloque 5 — DALCore / Generador (2 RESUELTAS, 2 delegadas)
- ✅ **Q-DALCORE-CONSUMERS** → Solo **WMSWebAPI** lo usa hoy. DMS NO. Cita: "hasta donde tengo entendido lo utiliza el WMSWebAPI". → ver `learnings/L-041-arch-dalcore-solo-wmswebapi.md`
- ✅ **Q-GENERADOR-UBICACION** → **Standalone, fuera de TOMWMS_BOF**, en repo separado (Carolina no recuerda nombre exacto). Mantenedores: **Erik + Efren**. → ver `learnings/L-042-arch-generador-codigo-standalone.md`
- 🔄 **Q-GENERADOR-INPUTS** → Carolina no sabe → **Delegada Erik/Efren**.
- 🔄 **Q-GENERADOR-WEB-VARIANTE** → Carolina no sabe → **Delegada Erik/Efren**.

### Bloque 6 — Multi-tenancy 3PL (1 RESUELTA, 2 delegadas)
- ✅ **Q-PROPIETARIO-AGNOSTICO** → Confirma re-respuesta Wave 8: clientes no-3PL usan **UN solo propietario default** en la tabla `propietarios`. WMS es 3PL-native by design. Cita: "Es correcto solo utilizan un propietario en la Tabla". → ver `learnings/L-048-proc-clientes-no-3pl-usan-un-propietario.md`
- 🔄 **Q-CEALSA-PREFACTURA-MODELO** → Cita: "Esto lo puede explicar bien Efren". → **Delegada Efren**.
- 🔄 **Q-CEALSA-RH-HR** → "Hasta donde sé no trabajamos nada de recursos humanos, pero habría que consultarle a Efren". → **Delegada Efren**.

### Bloque 7 — Recepción / capabilities (2 RESUELTAS, completo)
- ⭐ ✅ **Q-CAPABILITY-FLAG** → **HALLAZGO CRÍTICO WAVE 11**: **NO codifican por cliente, todo se parametriza** (sin `if Cliente.Codigo = "BYB"` por código). Tablas más parametrizadas: **Bodega, Producto, i_nav_config_enc**. Cita: "No codificamos por cliente, todo lo parametrizamos según corresponda por la funcionalidad que vayamos a incorporar al WMS". → ver `learnings/L-035-meta-no-codificar-por-cliente.md`. Reescribe cómo leer diferencias por cliente en TODO el código.
- ✅ **Q-LICENCIAS-MODELO-NUEVO** → Modelo nuevo = **licencias por Presentación** (MHS-driven). Parámetro nuevo `producto_presentacion.IdTipoEtiqueta` para diseño distinto al de licencia tradicional. Doble estreno con MHS (WMSWebAPI + licencias por presentación). → ver `learnings/L-043-arch-licencias-por-presentacion-mhs.md`

### Bloque 8 — Web BOF (0 RESUELTAS, 1 delegada)
- 🔄 **Q-WEB-BOF-STACK + Q-WEB-BOF-TIMELINE** → "No sé". → **Delegada Erik**.

### Bloque 9 — Bonus (5 RESUELTAS, 1 delegada)
- ✅ **Q-K7-ML-MODELO / Q-PERCEPTRON-USO-REAL** → **Perceptron abandonado**. Erik lo escribió para ubicación sugerida pero no escaló con BD grandes. Reemplazado por **método nuevo de ubicación sugerida más eficiente**. Cita Carolina: "Erik desarrolló esta neuronita... no fue muy funcional y creó un nuevo método". → ver `learnings/L-045-arch-perceptron-k7-abandonado.md`
- ✅ **Q-MAMPA-MERMA-CARNE-FLUJO** → MAMPA **NO vende carne**: vende ropa, zapatos, electrodomésticos, vitaminas. Caso "merma de carne" probablemente es **La Cumbre** (perecederos). → ver `learnings/L-044-client-mampa-erp-sap-hana-y-no-vende-carne.md`
- ✅ **Q-MAMPA-ERP** → **SAP HANA**. Confirma L-025: el prefijo `Nav` cubre también SAP HANA. → ver `learnings/L-044`
- ✅ **Q-MAMPA-CEDIS-PANTALLA-LEGACY** → **CEDIS no es legacy** del WMS, es **terminología cliente** = Centro de Distribución (recepción de proveedores + costeo). Vivo, soportado. → ver `learnings/L-047-meta-cedis-no-es-pantalla-legacy.md`
- ✅ **Q-K7-OC-TIPOS** → **18 tipos enumerados** (códigos 1-18, faltan 6/9/11/12). Catálogo más rico del brain hasta ahora (Almacén Fiscal, Pólizas, NC, Factura Reserva Proveedor). → ver `learnings/L-046-client-k7-tipos-oc-enumerados.md`
- 🔄 **Q-WMS-EXE-CONFIG-EN-WSHHRN** → "No sabría responder esta pregunta". → **Delegada Erik**.

---

## Q-* NUEVAS abiertas Wave 11 (van a bloque 14-15 del cuestionario)

~45 preguntas nuevas derivadas de los 14 learnings L-035..L-048. Catalogadas en
`_index/WAVE-11-CAROLINA-BLOQUES-3-A-9.md` sección "Q-* NUEVAS". Se transcribirán
formalmente al cuestionario en próximo refactor (Wave 12).

Subset crítico (alta prioridad):
- Q-PARAMETROS-CATALOGO: catálogo exhaustivo de banderas en Bodega, Producto, i_nav_config_enc, Empresa.
- Q-MHS-TRASLADOS-STATUS: status del bug que Erik está depurando antes de 20-ago.
- Q-MHS-WEBAPI-ENDPOINTS-CHECKLIST: 9 maestros + 4 trans, qué endpoints faltan.
- Q-STOCK-EN-LINEA-DEEP-FLOW: el reporte "super importante" de CEALSA.
- Q-GENERADOR-REPO-EXACTO: dónde vive el generador de código (Erik).
- Q-CUMBRE-MERMA-CARNE-FLUJO: confirmar si es Cumbre y mapear flujo.
- Q-NAV-CONFIG-ENC-MULTI-ERP: cómo `i_nav_config_enc` discrimina SAP HANA vs NAV vs IMS4.
- Q-MATRIZ-3PL-POR-CLIENTE: completar matriz 3PL/no-3PL para todos los clientes.

---

## Total Q-* RESUELTAS acumuladas: **~50 / 90 (56%)**

Distribución por wave:
- Wave 7: 4 RESUELTAS (Q-LP-WHEN-DESTROYED, Q-LP-MERGE-EN-DESTINO, Q-CAPABILITY-FLAG-prelim, Q-CONTROL-LOTE-TABLA)
- Wave 8: 4 RESUELTAS (Q-CLAVAUD-MEANING, Q-MI3-IDENTIDAD, Q-UMB-CONCEPT, Q-PROPIETARIO-AGNOSTICO-prelim)
- Wave 9: 5 RESUELTAS (Q-EXPLOSION-EXISTE, Q-RESTRICCION-UBICACION-CLIENTE, Q-LOTE-NUM-EXISTE, Q-TOLERANCIA-MULTI-NIVEL, Q-FROM-CHANNEL-FUNCTIONS)
- Wave 10: 11 RESUELTAS (Bloques 1 + 2 parciales)
- **Wave 11: ~17 RESUELTAS + 8 delegadas a Erik/Efren** (Bloques 2-cierre, 4 completo, 5 parcial, 6 parcial, 7 completo, 9 mayoría)
- Pre-Wave 7 (auto-resueltas con SQL/GREP): ~8

**Cambio crítico de interpretación Wave 11**: con L-035 confirmado, **Q-CAPABILITY-FLAG queda upgradeada de "RESUELTA preliminar Wave 7" a "RESUELTA definitiva Wave 11"** con cita literal de Carolina.

Pendientes principales:
- **Bloque 3 completo** (9 Q-*) → Erik/Efren (portal CEALSA, DMS, repos vacíos).
- **Bloque 8** (Q38) → Erik (web BOF stack/timeline).
- **Bloques 10-13** del cuestionario original (~28 Q-*) → no tocados aún.
- **Bloque 14 nuevo** (Q77-Q90 Wave 10) + **bloque 15 nuevo** (~45 Q-* Wave 11).
- Delegadas a Efren: Q15 (ChatGPT-Service), Q31, Q32 (Generador inputs/web), Q34 (Prefactura), Q35 (RH), Q-F bonus (WMS.exe.config).
- Delegadas a Erik: Q16-Q24 (Portal/DMS), Q38 (Web BOF), Q-WMS-EXE-CONFIG.
