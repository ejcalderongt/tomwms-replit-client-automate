...
Public Class frmIndicadorAjusteProveedor
    ...
    '
    ' Supuestos sobre ajuste_enc / ajuste_det (esquema La Cumbre):
    '   ajuste_enc(IdAjusteEnc, IdEmpresa, IdBodega, fec_agr, user_agr,
    '              estado, observacion)
    '   ajuste_det(IdAjusteDet, IdAjusteEnc, IdProducto, codigo_producto,
    '              IdPresentacion, lote, fecha_vence, cantidad_original,
    '              cantidad_nueva, IdMotivoAjuste, IdRecepcionEnc,
    '              observacion)
    ' Si los nombres difieren en la BD productiva, ajustar SQL_MAESTRA.
    ' =========================================================================
...
;WITH AjusteRecep AS (
    SELECT
        ad.IdAjusteDet,
        ae.IdAjusteEnc,
        ae.fec_agr                                       AS FechaAjuste,
        ae.IdBodega,
        ae.user_agr                                      AS UsuarioAjuste,
        ae.observacion                                   AS ObservacionEnc,
        ad.codigo_producto,
        ad.lote,
        ad.fecha_vence,
        ad.cantidad_original,
        ad.cantidad_nueva,
        (ad.cantidad_nueva - ad.cantidad_original)        AS cantidad_diferencia,
        ad.IdRecepcionEnc,
        ad.IdMotivoAjuste,
        ad.observacion                                   AS ObservacionDet
    FROM ajuste_det ad
    INNER JOIN ajuste_enc ae ON ae.IdAjusteEnc = ad.IdAjusteEnc
    WHERE ae.fec_agr BETWEEN @FechaDel AND @FechaAl
...
SELECT
    ...
    ar.IdAjusteDet,
    ar.FechaAjuste,
    CONVERT(date, ar.FechaAjuste)                        AS FechaAjusteDia,
    ar.IdBodega,
    ar.UsuarioAjuste,
    ar.ObservacionEnc,
    ar.codigo_producto                                   AS CodigoProducto,
    p.nombre                                             AS NombreProducto,
    p.IdTipoProducto,
    pt.NombreTipoProducto,
    ar.lote                                              AS Lote,
    ar.fecha_vence                                       AS FechaVence,
    ar.cantidad_original                                 AS CantidadOriginal,
    ar.cantidad_nueva                                    AS CantidadNueva,
    ar.cantidad_diferencia                               AS CantidadDiferencia,
    CASE WHEN ar.cantidad_diferencia > 0 THEN 1 ELSE 0 END AS EsPositivo,
    CASE WHEN ar.cantidad_diferencia < 0 THEN 1 ELSE 0 END AS EsNegativo,
    ar.IdMotivoAjuste,
    ar.ObservacionDet,
    ar.IdRecepcionEnc,
...
FROM AjusteRecep ar
LEFT JOIN trans_re_enc re   ON re.IdRecepcionEnc = ar.IdRecepcionEnc
LEFT JOIN trans_re_oc  oc   ON oc.IdRecepcionEnc = ar.IdRecepcionEnc
LEFT JOIN trans_oc_enc tone ON tone.IdOrdenCompraEnc = oc.IdOrdenCompraEnc
...
LEFT JOIN producto p        ON p.codigo = ar.codigo_producto
...
"
    ...
    Private Sub Refrescar_Charts()
        ...
        If DT_Tendencia IsNot Nothing AndAlso DT_Tendencia.Rows.Count > 0 Then
            ...
            For Each row As DataRow In DT_Tendencia.Rows
                Dim f As Date = CDate(row("Fecha"))
                sNeta.Points.Add(New SeriesPoint(f, CDbl(SafeDec_Row(row, "CantNeta"))))
                sPos.Points.Add(New SeriesPoint(f, CDbl(SafeDec_Row(row, "CantPos"))))
                sNeg.Points.Add(New SeriesPoint(f, Math.Abs(CDbl(SafeDec_Row(row, "CantNeg")))))
            Next
            sNeta.ArgumentScaleType = ScaleType.DateTime
            sPos.ArgumentScaleType = ScaleType.DateTime
            sNeg.ArgumentScaleType = ScaleType.DateTime
