...
## Actualización del 25/09/2026: detalle documental e Indicadores

La lista de ingresos navegaba a `/ingresos/detalle/:codigo` y la de salidas a `/salidas/detalle/:idDespachoEnc`; ninguna ruta existía en React Router. Las rutas correctas son `/detalle-ingreso/:id` y `/detalle-salida/:id`. En salidas, `correlativo` corresponde a `IdPedidoEnc` y es el ID que espera `detalle-pe`; `idDespachoEnc` identifica otra entidad. Las dos páginas de detalle también necesitaban leer el parámetro `id` del router. El test de navegador en `TOMWMSReact/tests/document-detail-navigation.test.mjs` reprodujo ambos fallos antes de publicar la corrección.

Después de arreglar el clic, `GET /api/sync/ingresos/229/detalle-oc` devolvía 500 aunque SQL tenía siete líneas. `clsBeTrans_oc_det` define `Nombre_propietario` y `Nombre_Propietario`; ambos nombres colisionan con las opciones JSON de la API. `SyncIngresosController` ahora proyecta solo los campos escalares consumidos por la pantalla. El documento 229 devuelve 200 y siete líneas; el 19171 devuelve una. La salida 49422 devuelve seis. `TOMWMS_BOF/WMSWebAPI/tests/Test-PortalDetails.ps1` valida ambos endpoints reales; el test de React incluye además la renderización real del ingreso. Mantener pruebas de navegador con datos simulados y una prueba contra la API real: la primera no detectó el 500 de serialización.

Se deshabilitó `indicadores` en `portal_propietario_modulo` para los 3,703 propietarios de `IMS4MB_CEALSA_QAS`; quedaron cero habilitados. Un login nuevo de Manuchar no devuelve el módulo ni `indicadores.ver`; inventario, ingresos y salidas permanecen habilitados. Los scripts versionados están en `TOMWMS_BOF/WMSWebAPI/Database/`: la migración transaccional y el seed que deja Indicadores apagado para propietarios futuros. Las sesiones abiertas necesitan volver a iniciar sesión para actualizar el menú. Los endpoints KPI legacy siguen siendo accesibles directamente; esta desactivación gobierna los permisos del portal.

Publicaciones IIS del día: React en `C:\Sites\PortalDMS` y API en `C:\inetpub\wwwroot\WMSWebAPI2`, conservando sus configuraciones. Respaldos locales privados: `deploy-backup-20260925-document-navigation` y `deploy-backup-20260925-ingreso-detail`. No subir respaldos ni secretos a Git.
