---
id: CASO-01-1_IDEAL_20231002011101
tipo: test-scenario
estado: vigente
titulo: Legacy CASO 1 — FEFO simple desde almacenamiento (ALM más corta que picking)
clientes: [idealsa]
ramas: [dev_2028_merge]
modulo: [reservation]
tags: [test-scenario, cliente/idealsa, modulo/reservation]
---

# Legacy CASO 1 — FEFO simple desde almacenamiento (ALM más corta que picking)

> Origen: `TOMWMS_BOF` rama `dev_2028_merge` archivo `/TOMIMSV4/DAL/Transacciones/Stock_Reservado/clsLnStock_res_Partial.vb` funcion `Ejecuta_QA_CASO_1_IDEAL_20231002011101` líneas 13119-13347.

## Mapeo a escenario canónico

| Campo | Valor |
|---|---|
| Escenario nuevo | `RES-001` |
| Categoría | `fefo-basico` |
| Cliente origen del caso | `IDEAL` |
| Vigente para | (ver `brain/test-scenarios/_matrix/compatibility.md`) |

## Resumen del caso (extraído del summary VB.NET)

> <summary>
> CASO#1 - IDEAL_20231002011101:
> Hay cajas en almacenamiento que vencen primero Vrs las que están en zona de picking o ubicación de picking.
> Modelado de inventario.
> Insertar una caja en ubicación de almacenaje con fecha corta. 01/01/2026
> Insertar una caja en ubicación de picking con fecha superior.  02/01/2026
> Resultado esperado: Reservar cajas desde almacenamiento, respetando el producto que vence antes.
> </summary>
> <returns></returns>

## Setup (datos embebidos)

| Campo | Valor |
|---|---|
| Producto | `47022` |
| Lotes | `CASO_USO1_MI3_AUTOMATE` |
| Fechas `New Date(y,m,d)` | `2026,1,1` |
| Tipo documento | `Pedido_De_Cliente` |
| Owner / Propietario | `01` |
| Cantidad solicitada | _(ver código)_ |
| Flags BeConfig accedidos | `IdProductoEstado`, `IdPropietario` |
| Función orquestadora | `clsLnI_nav_ped_traslado_enc.Importar_Pedido_Cliente_A_Tabla_Intermedia` |

## Acción

Ejecutar `Ejecuta_QA_CASO_1_IDEAL_20231002011101(IdBodega, IdUsuario, ByRef Resultado)`.

## Expected (del summary)

- Resultado esperado: Reservar cajas desde almacenamiento, respetando el producto que vence antes.

## Notas para portar al bridge

1. **Producto y owner están hardcoded** en el legacy (`47022` y `"01"`). El escenario canónico debe parametrizarlos.
2. **El lote** sigue patrón `CASO_USOnn_MI3_AUTOMATE`. En el bridge el lote también debe ser parámetro del escenario.
3. **Fechas relativas**: este caso usa fechas absolutas (2026,1,1). El bridge debe permitir expresarlas como `hoy + N días` para que no caduquen.
4. **Flag de rechazo por incompleto** y demás banderas se leen desde `clsLnConfiguracion_qa.Obtiene_Configuracion(IdBodega)`. En el bridge esto se mapea a `clients/<slug>/flags.yaml`.

## Referencia

- Función VB: [`/TOMIMSV4/DAL/Transacciones/Stock_Reservado/clsLnStock_res_Partial.vb` líneas 13119-13347](https://dev.azure.com/ejcalderon0892/TOMWMS_BOF/_git/TOMWMS_BOF?path=/TOMIMSV4/DAL/Transacciones/Stock_Reservado/clsLnStock_res_Partial.vb&version=GBdev_2028_merge)
