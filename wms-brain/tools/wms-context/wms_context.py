...
DEFAULT_EMBEDDINGS_URL = "http://127.0.0.1:1234/v1"
...
def discover_embedding_model(base_url: str, timeout: int) -> str | None:
    try:
...
def embedding_vector(base_url: str, model: str, text: str, timeout: int) -> list[float] | None:
...
def load_semantic_index(repo_root: Path) -> list[dict]:
    index_path = repo_root / "tools" / "wms-embeddings" / "out" / "chunks.jsonl"
...
def semantic_related_files(repo_root: Path, task: str, top_k: int = 4) -> list[str]:
    rows = load_semantic_index(repo_root)
    if not rows:
        return []
    model = os.getenv("EMBEDDINGS_MODEL") or discover_embedding_model(os.getenv("EMBEDDINGS_BASE_URL", DEFAULT_EMBEDDINGS_URL), timeout=20)
    if not model:
        return []
    query_vec = embedding_vector(os.getenv("EMBEDDINGS_BASE_URL", DEFAULT_EMBEDDINGS_URL), model, task, timeout=20)
    if not query_vec:
        return []
    scored = []
    for row in rows:
        score = cosine_similarity(query_vec, row.get("embedding") or [])
        scored.append((score, row["path"]))
    scored.sort(reverse=True)
    return unique_keep_order([path for _, path in scored[:top_k]])


def semantic_related_chunks(repo_root: Path, task: str, top_k: int = 4) -> list[dict]:
    rows = load_semantic_index(repo_root)
    if not rows:
        return []
    base_url = os.getenv("EMBEDDINGS_BASE_URL", DEFAULT_EMBEDDINGS_URL)
    model = os.getenv("EMBEDDINGS_MODEL") or discover_embedding_model(base_url, timeout=20)
    if not model:
        return []
    query_vec = embedding_vector(base_url, model, task, timeout=20)
    if not query_vec:
        return []
    scored = []
    for row in rows:
...
def select_context(repo_root: Path, task: str) -> tuple[list[str], list[RouteEntry]]:
    index_path = repo_root / "brain" / "agents" / "_index.yml"
    if not index_path.exists():
        selected_files = semantic_related_files(repo_root, task)
        return unique_keep_order(selected_files), []

    always_load, domains, clients = parse_index(index_path)
    ...
    selected_files.extend(semantic_related_files(repo_root, task))
...
def build_semantic_bundle(repo_root: Path, task: str, max_chars: int = 12000) -> str:
    chunks = semantic_related_chunks(repo_root, task, top_k=4)
    if not chunks:
        return ""
    parts = ["## Contexto semantico relevante"]
    for chunk in chunks:
        parts.extend(
            [
                f"### {chunk['path']}",
...
def main() -> int:
    ...
    semantic_bundle = build_semantic_bundle(repo_root, args.task)
    if semantic_bundle:
        source_bundle = semantic_bundle + "\n\n" + source_bundle
